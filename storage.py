...
def _derive_key_from_password(password: str) -> bytes:
    # Simple KDF using password bytes - for production use a proper KDF (scrypt/PBKDF2)
    # We'll keep it minimal here and rely on user passphrase entropy.
...
class TokenStore:
    ...
    def set(self, data: dict, fallback_password: Optional[str] = None):
        ...
        if not fallback_password:
            raise RuntimeError("Keyring not available. Provide a fallback password to encrypt tokens.")
    ...
    def get(self, fallback_password: Optional[str] = None) -> Optional[dict]:
        ...
        if not FALLBACK_FILE.exists():
            return None
        if not fallback_password:
            raise RuntimeError("Keyring not available. Provide fallback password to decrypt tokens.")
        key = _derive_key_from_password(fallback_password)
        f = Fernet(key)
